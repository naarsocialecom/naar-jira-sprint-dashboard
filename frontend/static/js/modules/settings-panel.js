// ============================================================
// SETTINGS-PANEL.JS — 3-tab settings drawer
//   Tab 1: Display  (sort order, legend style)
//   Tab 2: Columns  (epic table extra columns)
//   Tab 3: Colors   (bar chart + heat colors with hex input)
// ============================================================

import { Settings, saveSettingsToStorage } from './state.js';

// ── Available extra columns for the epic table ───────────────
export const EPIC_EXTRA_COLUMNS = [
    { key: 'issueCount', label: 'Issue Count'  },
    { key: 'assignee',   label: 'Assignee'     },
    { key: 'dueDate',    label: 'Due Date'      },
    { key: 'sprint',     label: 'Sprint Name'   },
    { key: 'priority',   label: 'Priority'      },
    { key: 'components', label: 'Components'    },
];

export const EpicColumns = {
    visible: new Set(['issueCount']),
};
...
export function saveEpicColumnsToStorage() {
    try {
        localStorage.setItem('naar_epic_columns', JSON.stringify([...EpicColumns.visible]));
    } catch (_) {}
}
    ...
    <div id="settingsPanel" class="settings-panel" role="dialog" aria-label="Dashboard Settings">
        ...
        <div class="settings-body settings-tab-pane active" id="tab-display">
            <div class="settings-section">
                ...
                <div class="settings-radio-group">
                    <label class="settings-radio-option">
                        ...
                        <span class="radio-label">High to Low</span>
                    </label>
                    <label class="settings-radio-option">
                        <input type="radio" name="sortOrder" value="low-to-high">
                        <span class="radio-custom"></span>
                        <span class="radio-label">Low to High</span>
                    </label>
                </div>
            </div>
            <div class="settings-section">
                ...
                <div class="settings-radio-group">
                    <label class="settings-radio-option">
                        ...
                        <span class="radio-custom"></span>
                        <span class="radio-label">Pills (compact)</span>
                    </label>
                    <label class="settings-radio-option">
                        <input type="radio" name="legendStyle" value="table">
                        <span class="radio-custom"></span>
                        <span class="radio-label">Table (S.No · Title · Count)</span>
                    </label>
                </div>
            </div>
            <div class="settings-section">
            ...
            <div class="settings-section settings-section-reset">
                <button id="settingsResetBtn" class="settings-reset-btn">
                    <i class="fas fa-undo-alt"></i> Reset Display
                </button>
            </div>
        </div>
        ...
        <div class="settings-body settings-tab-pane" id="tab-columns">
            <div class="settings-section">
                ...
                <p class="settings-hint">Epic Key &amp; Summary are always shown. Toggle extras below.</p>
                <div class="column-toggle-list" id="epicColumnToggles"></div>
            </div>
            <div class="settings-section settings-section-reset">
                <button id="columnsResetBtn" class="settings-reset-btn">
                    <i class="fas fa-undo-alt"></i> Reset Columns
                </button>
            </div>
        </div>
        ...
        <div class="settings-body settings-tab-pane" id="tab-colors">
            <div class="settings-section">
                <div class="settings-section-label"><i class="fas fa-palette"></i> Issue Type Chart Colors</div>
                <div class="color-grid" id="issueTypeColorGrid"></div>
            </div>
            <div class="settings-section">
                <div class="settings-section-label"><i class="fas fa-palette"></i> Component Chart Colors</div>
            ...
            <div class="settings-section settings-section-reset">
                <button id="colorsResetBtn" class="settings-reset-btn">
                    <i class="fas fa-undo-alt"></i> Reset Colors
                </button>
            </div>
        </div>

        <div class="settings-footer">
            <button id="settingsApplyBtn" class="settings-apply-btn">
...
export function applyStatusColorsToDOM() {
    STATUS_COLOR_DEFS.forEach(def => {
        const override = StatusColors[def.key];
        const bg   = override?.bg   || def.bg;
        const text = override?.text || def.text;
        // Inject a CSS variable override via a style block
        let styleEl = document.getElementById(`sc-${def.key}`);
        if (!styleEl) {
            styleEl = document.createElement('style');
            styleEl.id = `sc-${def.key}`;
            document.head.appendChild(styleEl);
        }
        styleEl.textContent = `.${def.key} { background: ${bg} !important; color: ${text} !important; }`;
    });
}
...
function buildStatusColorList() {
    ...
    STATUS_COLOR_DEFS.forEach(def => {
        ...
        row.innerHTML = `
            <span class="status-pill ${def.key}" style="background:${bg};color:${text};min-width:120px;text-align:center;">
                ${def.label}
            </span>
            <div class="status-color-pickers">
                <label class="status-color-picker-label" title="Background">
                    BG
                    <input type="color" data-status-key="${def.key}" data-type="bg" value="${bg}">
                    <input type="text"  class="color-hex-input" data-status-key="${def.key}" data-type="bg" value="${bg}" maxlength="7" placeholder="#000000">
                </label>
                <label class="status-color-picker-label" title="Text color">
                    Text
                    <input type="color" data-status-key="${def.key}" data-type="text" value="${text}">
                    <input type="text"  class="color-hex-input" data-status-key="${def.key}" data-type="text" value="${text}" maxlength="7" placeholder="#000000">
                </label>
            </div>
        `;
        // Wire native ↔ hex sync for both pickers
        row.querySelectorAll('label.status-color-picker-label').forEach(lbl => {
            const native = lbl.querySelector('input[type=color]');
            const hex    = lbl.querySelector('input[type=text]');
            const pill   = row.querySelector(`.${def.key}`);
            native.addEventListener('input', () => {
                hex.value = native.value;
                updatePillPreview(pill, row, def.key);
            });
...
function updatePillPreview(pill, row, key) {
    const bgNative   = row.querySelector(`input[type=color][data-status-key="${key}"][data-type="bg"]`);
    const textNative = row.querySelector(`input[type=color][data-status-key="${key}"][data-type="text"]`);
    if (pill && bgNative && textNative) {
        pill.style.background = bgNative.value;
        pill.style.color      = textNative.value;
    }
}

function collectStatusColors() {
    document.querySelectorAll('#statusColorList label.status-color-picker-label').forEach(lbl => {
        const native = lbl.querySelector('input[type=color]');
        if (!native) return;
...
// ── Color grid builder (with hex input) ─────────────────────
function wireColorPair(nativePicker, hexInput) {
    nativePicker.addEventListener('input', () => { hexInput.value = nativePicker.value; });
    hexInput.addEventListener('input', () => {
        const v = hexInput.value.trim();
        if (/^#[0-9a-fA-F]{6}$/.test(v)) nativePicker.value = v;
    });
...
export function initSettingsPanel() {
    ...
    if (headerRight) {
        ...
        btn.id = 'settingsTriggerBtn';
        btn.className = 'settings-trigger-btn';
    ...
    document.getElementById('settingsResetBtn')?.addEventListener('click', () => {
        Settings.sortOrder = 'high-to-low'; Settings.legendStyle = 'table'; syncDisplayForm();
    });
